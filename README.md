...
| Perfil dos programas | `D:\Perfil`: uma junção por programa (Chrome, Discord, Spotify, VS Code, Obsidian, OBS, gh, `~\.claude`, `~\.ssh`, `~\.config`) e `Steam\config` + `Steam\userdata` em `D:\Jogos\Steam\_perfil`. Nasce antes de o programa ser instalado, então ele já grava em D: sem saber; depois da formatação a pasta está lá e a preferência volta. O que a DPAPI da conta cifra não volta, por desenho do Windows: cookies e sessões do Chrome, token do Discord e do Spotify, credencial do git pedem login de novo (as senhas estão no Proton Pass). Programa novo que guarde preferência ganha uma linha na etapa 7 |
